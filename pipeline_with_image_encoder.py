...
class StableDiffusionImg2ImgPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromCkptMixin):
    ...
    def __call__(
        ...
        self.check_inputs(strength, callback_steps)
        ...
        image_embeds = processor(image, return_tensors="pt")
        image_embeds = clip_model.get_image_features(**image_embeds).unsqueeze(0).to(device)
        ...
        latents = self.prepare_latents(
            image, latent_timestep, batch_size, num_images_per_prompt, device, generator
        )
        ...
        if not output_type == "latent":
            ...
            image, has_nsfw_concept = self.run_safety_checker(image, device, image_embeds.dtype)
        else:
            image = latents
            has_nsfw_concept = None
